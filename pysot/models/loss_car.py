...
class SiamCARLossComputation(object):
    ...
    def compute_targets_for_locations(self, locations, labels, gt_bbox):
        ...
        s1 = reg_targets_per_im[:, :, 0] > 0*((bboxes[:,2]-bboxes[:,0])/2).float()
        s2 = reg_targets_per_im[:, :, 2] > 0*((bboxes[:,2]-bboxes[:,0])/2).float()
        s3 = reg_targets_per_im[:, :, 1] > 0*((bboxes[:,3]-bboxes[:,1])/2).float()
        s4 = reg_targets_per_im[:, :, 3] > 0*((bboxes[:,3]-bboxes[:,1])/2).float()
