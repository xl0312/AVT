...
def xcorr_fast(x, kernel):
    ...
    return po
def xcorr_depthwise_time(x, kernel):
...
class timemodule(nn.Module):
    def __init__(self, inchannels, outchannels):
        ...
        for i in range(4):
            ...
            time_tower.append(nn.BatchNorm3d(256, 256))
            time_tower.append(nn.ReLU())
        self.add_module('time_tower', nn.Sequential(*time_tower))
    def forward(self, x):
        time_feature = self.time_tower(x)
        return time_feature